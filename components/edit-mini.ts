...
export class EditMini extends LitElement {
  ...
  _approveMini() {
    this.mini.status = Status.Approved;
    this.mini.base64Image = getImage();
    updateMini(this.mini);
  }

  _rejectMini() {
    this.mini.status = Status.Rejected;
    updateMini(this.mini);
  }

  _loadModel() {
    renderFile(this.mini, "model");
  }
        ...
        return html`
          <div class="imposter" style="background-color:white; border-style:solid;">
            ...
            <div class="row">
              ...
              <div class="flex-item">
                <div id="status">${this.mini.status}</div>
